...
const wavestartbtn = document.getElementsByClassName("mainbutton")[0];
...
let monsters = [];
for (let i = 0; i < 3; i++) {
  let monster = new Monster(-50, 150, 10, 2);
...
for (let i = 0; i < 2; i++) {
  let tower = new Tower(
    Tile.tiles[12 + i * 10].x,
    Tile.tiles[12 + i * 10].y,
    Tile.tiles[12 + i * 10].posx,
    Tile.tiles[12 + i * 10].posy,
    100
  );
  towers.push(tower);
}
...
//     let tower = new Tower();
//     towers.push(tower);
//     console.log("install tower : "+tower);
//     console.log("tower list : "+ towers);
//   }
// });
...
function searchMonster(tower) {
  let range = 100;

  // 범위 내 몬스터를 필터링하여 찾기
  let target = monsters.find(m => 
    tower.posx - range <= m.x && tower.posx + range >= m.x &&
    tower.posy - range <= m.y && tower.posy + range >= m.y
  );

  return target || null; // 타겟이 없으면 null 반환
}

function atkTower() {
  towers.forEach(tower => { // 모든 타워 순회
    let target = searchMonster(tower); // 각 타워마다 적 탐색

    if (target) {
      let p = new Projectile(tower.posx, tower.posy, target, 10);
      projectiles.push(p);
    }
  });
  ...
  projectiles = projectiles.filter((p) => {
    ...
    if (distance > 5) {
      ...
      // 타겟이 죽었으면 배열에서 제거
      if (p.target.hp <= 0) {
        let index = monsters.indexOf(p.target);
        if (index !== -1) {
